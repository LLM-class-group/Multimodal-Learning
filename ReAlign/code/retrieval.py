...
os.environ["SERPER_API_KEY"] = "9d0cf1cb8c5fe3bc68ca175585adb3f12a24e9fd"
...
def retrieval():
    ...
    need_retrieval_data = [d for d in original_data if d['items']
                           [0]["category"] in RAG_TASKS and len(d['items']) == 2]
    logger.info(f'{len(need_retrieval_data)} samples have retrieval task')

    retrieval_data = []

    for ret_d in need_retrieval_data:
        if ret_d['items'][0].get('evidence') and ret_d['items'][0]['evidence'] != "No good Google Search Result was found":
            continue
        retrieval_data.append(ret_d)

    logger.info(
        f'{len(need_retrieval_data) - len(retrieval_data)} samples have been retrieved')
    ...
    for i in tqdm(range(num_batches)):
        ...
        queries = [q['items'][0]["value"]
                   for q in retrieval_data[batch_start:batch_end]]
        ...
        for idx, res in enumerate(responses):
            ...
            retrieval_data[batch_start +
                           idx]['items'][0]['evidence'] = evidences

        for ret_d in retrieval_data[batch_start:batch_end]:
            original_data_dict[ret_d['id']] = ret_d

        with open(args.output_path, 'w') as fp:
            data_list = list(original_data_dict.values())
            fp.write(json.dumps(data_list, indent=4, ensure_ascii=False))
